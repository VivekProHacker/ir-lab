...
public class DRelevance extends Indexer {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		/*
		if(frequencyData.containsKey("एनवायरमेंट")){
			ArrayList<termfreq> obj1=frequencyData.get("एनवायरमेंट");
			for(int x=0;x<frequencyData.get("एनवायरमेंट").size();x++){
			String textFilename=flist[obj1.get(x).doc_id].getName();
			System.out.println("Doc"+obj1.get(x).doc_id+" File NAme"+textFilename);
			}
	
	}
	*/
		Scanner keyboard = new Scanner(System.in);
		System.out.println("We are taking top three document as Relevant Documents.");
		Integer token1[]=top3;
		//System.out.println("Top 3:" + token1[0]);
		//String query2=keyboard.nextLine();
		//String token1[]=query2.split(" ");
		//sort an array.
		//Arrays.sort(token1);
		//Integer[] token2=new Integer[token1.length];
		//token2=parseInt(token1);
		double[] optQuery=new double[treeSize(frequencyData)];
		
		String[] token=getIntialQuery();
		
		optQuery=optimalQuery(token1,token,frequencyData,flist.length );
