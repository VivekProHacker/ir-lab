...
public static String[] rank( TreeMap<String,ArrayList > frequencyData,ArrayList<termfreq> doclist, String[] token,File[] flist)throws IOException, ClassNotFoundException{
		...
		Arrays.sort(idx,new Comparator<Integer>(){
			public int compare(final Integer o1,final Integer o2){
				...
				for(int i=z-1;i>=z-20;i--){
					...
					rankedDocs[fileSize-i-1]=flist[idx[i]].getName();
					//System.out.println(flist[idx[i]].getName());
				//	System.out.println("Rank "+ (fileSize-i-1)+" score " +rank[idx[i]] +" Doc Id" +flist[idx[i]].getName()+ "Title :"+word);
		//System.out.println("Rank "+ (fileSize-i-1)+" score " +rank[idx[i]] +" Doc Id" +((idx[i]) )+ "Title :"+word);
					}
